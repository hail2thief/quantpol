...
Some packages, like `{juanr}` cannot be installed using `install.packages()` because they are hosted on [Github](https://github.com/). To install `{juanr}`, you will first need to install `{devtools}` and then use `install_github()`, like so:
