...
Make a barplot that puts the baby's name on the y-axis, and the proportion of babies with that name (the variable is called `prop`) on the x-axis. Map the sex variable to the fill aesthetic to create separate bars for male and female babies. 
...
Are movies getting *longer* or *shorter* over time? Show the distribution of movie duration using a *histogram*, and map the `fill` aesthetic to `decade` so we can see the distribution within each decade. 
...
**Question 7: compare the distribution of movie length across decades. Are movies getting longer over time, shorter over time, or are the distributions pretty similar?**




Finally, let's look at who are the most/least *profitable* scary movie directors. First, we'll make a new dataset that only looks at scary movies, calculates each movie's profit (`gross` - `budget`) and finds the average profit for each director. To make this more manageable, let's look at directors who made at least 2 movies. 
...
directors_profit = movies %>% 
  ...
  filter(genre %in% c("Horror", "Thriller", "Mystery")) %>% 
  # calculate profit
  ...
  # include directors who made more than 1 movie
  filter(n_movies > 1)
