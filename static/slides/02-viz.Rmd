...
map president's **party** to the `color` aesthetic, and whether or not president served two terms to `shape`
...
```{r,echo = TRUE,out.width="70%"}
ggplot(elections_historic, aes(x = popular_pct, y = ec_pct, color = win_party, shape = two_term, label = winner_label)) + #<<
...
```{r, echo = TRUE, eval = FALSE}
# just if you're curious! Note you'd need to install the font "Fira Sans Condensed" on your computer
ggplot(elections_historic, aes(x = popular_pct, y = ec_pct, label = winner_label,
                               color = win_party, shape = two_term, 
                               label = winner_label)) + 
  geom_point() +  
  geom_text_repel(family = "Fira Sans Condensed") + 
...
???
What's going on here? all points above 50% EC, but not adams
not all points above 50% pop vote
