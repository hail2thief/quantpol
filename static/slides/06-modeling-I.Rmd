...
p1 = therm %>% 
  ...
  group_by(party_id) %>% 
  summarise(avg = mean(ft_muslim_2017, na.rm = TRUE)) %>% 
...
p2 = therm %>% 
  filter(party_id %in% c("Democrat", "Republican")) %>% 
  group_by(party_id) %>% 
  summarise(avg = mean(ft_jew_2017, na.rm = TRUE)) %>% 
...
.pull-left[
Models are ways of describing relationships (or correlations) between variables

In the social sciences, we are interested in describing how an **outcome** changes in response to a **treatment**
...
Say we want to model how changing a car's **weight** (treatment) impacts its **fuel efficiency** (outcome)
...
Strong, negative relationship (r = -.9), *heavier* cars have  *lower* fuel efficiency
...
It also gives us a "best guess" for the level of fuel efficiency for a given weight, even where we have *less* data
...
ggplot(mtcars, aes(x = wt, y = mpg)) + geom_point(color = blue, alpha = .8) + 
  labs(x = "Weight (1000 lbs)", y = "Miles/gallon (mpg)") + 
  geom_smooth(method = "lm") #<<
```
...
How far is each point from the line?  This is the **residual**
...
# 🚨Your turn🚨 Draw the lines

Using pen and paper, sketch the following lines: 
...
Then confirm your intuition with the R code
...
We can plug in whatever weight we want and get an estimate of fuel efficiency
  * weight = 3.1415 tons $\rightarrow$ $mpg = 37 + -5 \times 3.1415 = 21.2$
...
1. Using the fake dataset `df`, make a scatterplot and include a line of best fit. 

2. Now using the function I made, `line_of_best_fit()`, try out different combinations of intercepts and slopes and see how the sum of squared residuals varies. 

3. Convince yourself the line of best fit (in `true`) is the best. 
