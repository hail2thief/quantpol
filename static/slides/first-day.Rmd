...
ggplot(pDat, aes(x = factor(x), 
  ...
  labs(x = "Was the group decapitated?", 
       y = "Likelihood of group failure") + 
  theme(legend.position = "none", 
        panel.grid.minor = element_blank()) + 
  scale_color_manual(values = c(red, blue)) + 
  coord_flip() + 
  ggrepel::geom_label_repel(aes(label = c("Chance of failure: 2%", "Chance of failure: 10%")), nudge_x = -.2, family = "Fira Sans Condensed")
```
...
class: center, middle, inverse
# Does this mean decapitation **causes** break down?
...
Ignoring these differences will **confound** our estimates!
...
**Wrangling** data to answer the questions we care about

Using **models** to explore relationships in the political world
...
.pull-left[
- Able to work with data = marketable skill

- R is popular in industry

- Familiarity with R $\rightarrow$ other languages
...
I want students who are working hard to do well

--

Half done home work is **strictly better** than no homework
...
```{r spreadsheet}
who = read_csv("/Users/juan/Dropbox/Mac/Downloads/pol51-responses.csv") %>% 
  janitor::clean_names() %>% 
...
who %>% 
  ...
  tally() %>% 
  ggplot(aes(x = fct_reorder(what_year_are_you, n), y = n)) + 
  ggalt::geom_lollipop(size = 1.5, color = red) + 
  coord_flip() + 
...
```{r}
who %>% 
  select(`Experience coding` = how_much_experience_if_any_do_you_have_with_computer_programming_coding, `Experience data` = how_much_experience_if_any_do_you_have_with_working_with_data) %>% 
  pivot_longer(everything()) %>% 
  group_by(name, value) %>% 
  tally() %>% 
  ggplot(aes(x = value, y = n)) + 
  geom_col(fill = red, alpha = .9) + 
  hrbrthemes::theme_ipsum(base_family = "Fira Sans", 
                          base_size = 14,
                          grid = "Y") + 
...
```{r}
who %>% 
  select(what_if_anything_concerns_you_about_this_class) %>% 
  drop_na() %>% 
  slice(2, 4, 7, 55, 41) %>% 
