...
Multiple regression is just regression with *multiple* explanatory variables
...
We should add labels to the models: 
...
Get rid of the extra crap, keep only the sample size (`nobs`):
...
- Need math to fully grasp, but plots help:
]
...
Let's make up some data to show this: X has an effect of 8 on Z; Z has an effect of 4 on Y
...
What's the total effect of X on Y: just believe me, it is $8 \times 4 = 32$

Remember, with a **pipe** controlling for Z is a .shout[bad idea]

Let's fit two models, where one makes the mistake of controlling for Z

```{r, echo = TRUE}
right_model = lm(y ~ x, data = fake_pipe)
bad_control = lm(y ~ x + z, data = fake_pipe)
```
...
ggplot(collide_google, aes(x = social_skills, y = tech_skills)) + 
  ...
  theme_fancy + labs(x = "Social skill score", y = "Technical skill score") + 
  geom_smooth(method = "lm")
```
