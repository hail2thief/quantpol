...
--
.box-1.large.sp-after[The bootstrap]
...
How do we get a sense for our uncertainty in this one sample?
...
We need to simulate what it would look like if we had taken many samples from the population we are interested in 


--

We can take samples *from our dataset* that are the same size as the original, but **with replacement** (i.e., the same observation can appear multiple times)

--

This **mimics** how samples from a population vary, and gives us a sense for how much the estimate we care about might vary
...
If we were sampling these delicious fruits: 
...
* Don't have many samples? Well then, "pull yourself up by your bootstraps" and create some by sampling with replacement
* Use the distribution of these samples to quantify uncertainty
]
...
Last week, small sample, lots of reps, no replacement:
...
Bootstrap, same-size sample, lots of reps, with replacement: 

```{r, echo = TRUE, eval = FALSE}
gss_sm %>% 
  rep_sample_n(`size = 2867`, reps = 1000, `replace = TRUE`) %>% # size = nrow(gss_sm), with replacement
...
Let's bootstrap estimates of the average number of kids in the US, based on `gss_sm`: 
...
This is how much our estimates might vary from sample to sample: 
...
ggplot(boot_kids, aes(x = avg_kids)) + 
  ...
  geom_vline(xintercept = mean(gss_sm$childs, na.rm = TRUE), size = 2, color = blue)
```
...
One approach is to measure the "spread" of the distribution, using the standard deviation
...
3. Calculate and report the 95% confidence interval of your best guess. 
...
How much might our estimate of `childs ~ age` vary? We can look at the standard error: 
...
How much might our estimate of `childs ~ age` vary? We can look at the 95% confidence interval
...
# Reporting statistical significance: the stars


The *stars* (*) in regression output tell you about whether an estimate's confidence interval crosses the dreaded zero, through a statistic called the p-value

We didn't cover p-values, but they are just the mirror image of the confidence interval
