...
Boxplots compare the **distributions** of *continuous* variables across groups -- show us more than the barplot which only gives us a quantity for each group
...
tribble(~Graph, ~`aes()`, ~`geom_`, ~Purpose,
        ...
        "Barplot", "x = category, y = amount", "col()", "Compare", 
        "Boxplot", "x = category, y = amount", "boxplot()", "Compare distributions") %>% 
  knitr::kable()
```
...
ggplot(gap_07, aes(x = gdpPercap, y = lifeExp)) + theme_light() + 
  geom_point(size = 3, color = "orange", shape = 2, alpha = .8) #<<
```
...
library(ggridges)
ggplot(gapminder, aes(y = continent, x = lifeExp)) + theme_light() + 
    geom_density_ridges() #<<
```
...
```{r, echo = TRUE, out.width="60%"}
library(ggbeeswarm)
ggplot(gapminder, aes(x = continent, y = lifeExp)) + theme_light() + 
    geom_quasirandom() #<<
```

Beeswarm plots tell us something boxplots don't: the number observations by group
