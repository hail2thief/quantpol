...
Note that this is what the model output is giving us:
...
.small[
Intercept (set `optOut` = 0 or "off"): Country-years where people have to OPT IN to organ donation have, on average, 14.3 donations per million residents
]
...
And look at the output with `tidy()`:
...
Same as before, but now multiple groups, each with own "line of best fit"
...
It will then tell us how much higher/lower the other categories are, on average, *relative to that baseline*
