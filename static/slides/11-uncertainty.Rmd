...
The end: how confident are we in our estimates given that they *vary* from sample to sample? 
...
ggplot(gapminder::gapminder, aes(x = gdpPercap, y = lifeExp)) + 
  ...
  geom_smooth(color = blue, fill = blue) + theme_fancy + 
...
What's the relationship between sex and vote choice among all Americans? We can estimate that with `gss_sm` in this made-up world we're in:
...
ggplot(kids_10, aes(x = avg_kids)) + 
  ...
  theme_fancy + 
  labs(x = "Average number of kids in sample", 
       title = glue::glue("Size of each sample? N = {unique(kids_10$size)}")) + 
  geom_vline(xintercept = mean(gss_sm$childs, na.rm = TRUE), lty = 2, color = blue, 
             size = 3) + 
...
```{r}
ggplot(kids_20, aes(x = avg_kids)) + 
  geom_density(fill = red, color = "white") + 
  theme_fancy + 
  labs(x = "Average number of kids in sample", 
       title = glue::glue("Size of each sample? N = {unique(kids_20$size)}")) + 
  geom_vline(xintercept = mean(gss_sm$childs, na.rm = TRUE), lty = 2, color = blue, 
             size = 3) + 
  coord_cartesian(xlim = c(0, 4))
```
...
They become more and more concentrated around the population parameter...

```{r}
ggplot(kids_100, aes(x = avg_kids)) + 
  geom_density(fill = red, color = "white") + 
  theme_fancy + 
  labs(x = "Average number of kids in sample", 
       title = glue::glue("Size of each sample? N = {unique(kids_100$size)}")) + 
  geom_vline(xintercept = mean(gss_sm$childs, na.rm = TRUE), lty = 2, color = blue, 
             size = 3) + 
  coord_cartesian(xlim = c(0, 4))
```
...
```{r}
ggplot(kids_300, aes(x = avg_kids)) + 
  geom_density(fill = red, color = "white") + 
  theme_fancy + 
  labs(x = "Average number of kids in sample", 
       title = glue::glue("Size of each sample? N = {unique(kids_300$size)}")) + 
  geom_vline(xintercept = mean(gss_sm$childs, na.rm = TRUE), lty = 2, color = blue, 
             size = 3) + 
  coord_cartesian(xlim = c(0, 4))
```
...
Coefficient estimates become more precise as sample size increases:
...
.pull-right[
```{r, fig.cap="Good samples at Sam's club"}
...
But estimates will no longer be centered around population parameter; they will be **biased**
