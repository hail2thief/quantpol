...
Now, look at the correlation below: 
...
**Q2: Interpret the correlation between body mass (in grams), and bill depth (in millimeters)**
...
```{r}
lm(conflict ~ funding, data = aid_conf) %>% tidy()
```
...
```{r}
lm(conflict ~ funding,
   data = filter(aid_conf, corruption == "Low Corruption")) %>% tidy()

lm(conflict ~ funding,
   data = filter(aid_conf, corruption == "High Corruption")) %>% tidy()
```
...
```{r}
ggplot(aid_conf, aes(x = funding, y = conflict))  + 
  facet_wrap(vars(corruption), scales = "free") + 
  theme_bw() + 
  geom_point() + 
...
Look at the `county_data` dataset from `socviz`, which provides county-level information on vote results and other variables. 


```{r}
county_data = county_data %>% 
  drop_na(per_dem_2016)

county_data
```


Run a regression where you try to explain GOP performance in 2016 using the region of the country that the county is in. Note: to see what values a specific variable takes on, you can use: `data %>% distinct(variable)`

```{r}
lm(per_gop_2016 ~ census_region, data = county_data) %>% 
  tidy()
```

**Q8: Using SPECIFIC language, interpret the coefficient estimate for the intercept (i.e., you need to identify the region)**



**Q9: Based on the above, in which region did Republicans perform the worst?**
